...
### D-006 — Flujo de ramas: `dev` → PR → `main`, con `main` protegida
**Fecha**: 2026-07-15
`main` es la rama estable: solo recibe código vía pull request. El trabajo del día a día va en `dev` (o en ramas de feature que salgan de `dev`). Aunque el proyecto es de una sola persona, el PR es el punto donde se ejecuta el CI y se repasa la checklist del template (pin del vendor, parches al vendor, build nativo, GPLv3) — que es justo lo que la reproducibilidad del benchmark exige no saltarse por inercia.

**Configuración efectiva en GitHub** (protección clásica sobre `main`):
- Pull request obligatorio; `required_approving_review_count: 0` (un solo autor: exigir aprobación se auto-bloquearía).
...
- Status checks obligatorios: `core (nativo)` y `lint` (ver `.github/workflows/ci.yml`).

**CI**: compila **siempre nativo** (nunca `--target wasm32-*`), corre `cargo check/test`, un smoke test que ejecuta el spike y verifica que el AC 1 bus se alimenta con ext pwr, y `fmt`/`clippy` con `-D warnings` sobre nuestro crate (el vendor está fuera del workspace, así que sus warnings no entran). El submódulo de FBW (~600 MB) se clona shallow y las deps van cacheadas; aun así el primer run en frío es caro.
